    ...
    if ($handle->{dbh}) {
            ...
            < $conn_details->{connection_check_threshold}) 
        {
            ...
            if (_check_connection($handle->{dbh})) {
                ...
                $hooker->('database_connection_lost', $handle->{dbh});
...
sub _get_connection {
    ...
    $settings->{dbi_params}{HandleError} = sub {
        my ($error, $handle) = @_;
        $hooker->('database_error', $error, $handle);
    };
    ...
    if (!$dbh) {
        ...
        $hooker->('database_connection_failed', $settings);
        return;
